...
--------
  ...
  8.  Print the optimal parameters and final accuracy.
...
class HindcastOptimizer:
    ...
    params[2] : wind_direction_offset  Δθ ∈ [-30°, +30°]
        Rotational correction added to the ERA5 push-toward wind direction.
    ...
    def objective_function(self, params: np.ndarray) -> float:
        ...
        rotated_dir = (self.base_wind_dir + wind_dir_offset) % 360.0
...
def run_hindcast(
    ...
    Returns
    -------
    result dict with keys: best_params, best_error, iou, df_firms, landscape, ...
    """
    ...
    if using_shapefile:
        print("\n[1/7] Copernicus EMS shapefile provided — skipping NASA FIRMS fetch.")
        ...
        print("\n[2/7] Identifying State Zero (first detection) ...")
    ...
    print("\n[4/7] Fetching real terrain (Copernicus COP30 + CORINE) ...")
    ...
    if dem_file and corine_file:
        ...
        print("[4/7] Falling back to synthetic terrain (DEM/CORINE unavailable).")
    ...
    if using_shapefile:
        ...
        if df_after.empty:
            raise ValueError(
                "[5/7] No FIRMS detections after State Zero. "
        ...
        if df_window.empty:
            raise ValueError("[5/7] Still no FIRMS detections in window — check bbox.")
    ...
    print(f"\n[7/7] Reloading terrain at {FINE_SHAPE[0]}x{FINE_SHAPE[1]} "
          f"for final IoU run ...")
    ...
    # Re-apply best weather & optimised params to fine landscape
    ...
    # Wind: apply best_wind_mult and direction offset on the fine grid.
    ...
    fine_wind_dir = (land_fine.wind_dir + best_wind_dir_offset) % 360.0
    land_fine.set_wind(weather["wind_speed_ms"] * best_wind_mult, fine_wind_dir)
