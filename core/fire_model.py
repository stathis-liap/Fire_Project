...
class CellularAutomataFire:
    ...
    def _precompute_ros_grid(self) -> None:
        ...
        # 3. Reaction intensity — suppress overflow on Non_Combustible cells
        #    (sigma=1e-9, w0=0) which are masked out by valid_cells below.
        ...
        #     wind_u_grid / wind_v_grid are (rows, cols) float32 arrays (m/s).
        wind_u_grid, wind_v_grid = compute_wind_field(self.landscape, self.config)

        # Cache wind grids for the spotting model in step()
        self._wind_u_grid = wind_u_grid
        self._wind_v_grid = wind_v_grid

        # Pre-compute fuel-dependent Rothermel wind coefficients once (outside
        # the direction loop) — shapes are all (rows, cols).
        c_w = 7.47 * np.exp(-0.133 * sigma ** 0.55)
        b_w = 0.0252 * sigma ** 0.54
        e_w = 0.715 * np.exp(-3.59e-4 * sigma)

        # 5b. Calculate directional modifiers (Wind and Slope) for all 8 directions
        for i, (dr, dc) in enumerate(self._neighbors):
            ...
            # Per-cell midflame wind speed aligned with this spread direction.
            # dr_vec[0] = dc (East-West / column), dr_vec[1] = dr (N-S / row)
            # — matches the (wind_u = East-West, wind_v = North-South) convention
            # in landscape.py.  Result is a (rows, cols) array in m/s.
            dr_vec = np.array([dc, dr], dtype=np.float64)
            dr_vec /= np.linalg.norm(dr_vec)
            u_effective = wind_u_grid * dr_vec[0] + wind_v_grid * dr_vec[1]
            u_ft_min = np.maximum(0.0, u_effective) * _MS_TO_FTMIN   # (rows, cols)

            # phi_w is now a full (rows, cols) matrix — each cell gets the
            # correct Rothermel wind factor for its local terrain and canopy.
            # Clamp u_ft_min to Rothermel's calibrated wind range (0–1500 ft/min
            # ≈ 0–7.6 m/s midflame) to prevent extrapolation blow-up.
            ...
            # Hard cap at 1.0: physically a fire cannot cross more than one
            # full cell in a single time step (Courant-Friedrichs-Lewy condition).
    ...
    def _apply_spotting(self, burning_mask: np.ndarray,
        ...
        For each burning cell a small number of firebrands are lofted with
        probability P_loft.  Landing distance is drawn from a log-normal
        distribution parameterised by the local wind speed; the flight
        direction is the local downwind direction ± random angular scatter.
        Any firebrand that lands on unburned, combustible fuel deposits
        ignition heat (0.5 fraction) at that cell.
        """
        if not hasattr(self, "_wind_u_grid"):
            return  # not yet precomputed (called before first precompute)

        cell_m = self.config.CELL_SIZE_METERS
        rows, cols = burning_mask.shape

        # Row/col indices of active burning cells
        br, bc = np.where(burning_mask)
        if br.size == 0:
            return

        # Local wind at each burning cell
        wu = self._wind_u_grid[br, bc]   # East (+) / West (-)
        wv = self._wind_v_grid[br, bc]   # North (+) / South (-)
        wind_spd = np.hypot(wu, wv).clip(0.1)  # m/s, avoid div-by-zero

        # ── Lofting probability (capped at 8% per step to stay physical) ──
        p_loft = np.clip(0.003 * wind_spd, 0.0, 0.08)
        ...
        # ── Landing distance (log-normal, metres → cells) ─────────────────
        # mean_dist calibrated: ~150 m at 1 m/s, ~750 m at 5 m/s midflame
        mean_dist_m = np.maximum(200.0, 150.0 * wind_spd)
        sigma_ln    = 0.45          # spread of log-normal (dimensionless)
        mu_ln       = np.log(mean_dist_m) - 0.5 * sigma_ln ** 2
        dist_m      = np.random.lognormal(mu_ln, sigma_ln)
        dist_cells  = dist_m / cell_m

        # ── Flight direction: downwind + uniform ±20° scatter ──────────────
        base_angle  = np.arctan2(wv, wu)                     # radians
        scatter     = np.random.uniform(-np.pi / 9, np.pi / 9, br.size)  # ±20°
        ...
        land_c = bc + dc_land
        land_r = br - dr_land   # row index grows southward; subtract for north-up wind

        # ── Filter out-of-bounds and already-burning cells ──────────────────
        in_bounds = (
            (land_r >= 0) & (land_r < rows) &
        ...
        # ── Deposit heat (0.5 fraction per landing firebrand) ───────────────
